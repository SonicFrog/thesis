...
pipeline. % TODO: describe how this done
...
The key-value store is in this case what we call the user program, the
layer that comes last on top of the R2P2 stack. It registers a request
callback with the R2P2 server and parses the request payload to then
perform the requested operations.

The store supports arbitrary key and value size and types, though it
does no validation of any kind on either the keys or values it is
provided. It makes use of the hashmap mentioned in
~\ref{design-overview} and adds request parsing on top of the R2P2
server.
...
our system. Though not perfect by any stretch of the imagination it
outperforms memcached by a lot. % hopefully


\subsection{Further work}
ARP layer in the networking stack?
TCP stack
mention the weird bug when allocating new packets?
